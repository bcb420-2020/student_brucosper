...
es <- readRDS("annotated_expression_set.Rds")
...
The "BH" method was chosen as this seems to be generally the most appropriate for microarray data, as well as being less stringent.  


#### How many genes passed correction
A total of `r length(which(topfit$adj.P.Val < 0.05))` genes passed correction. It should be noted that in the paper the authors used log odds instead of P-values, which would give `r length(which(topfit$B > 0))` instead.
